...
def test_ActionDataset_frame_extraction_inbound_frame_count(
    input_sequences: Path, frames_to_extract: int
):
    ...
    with patch.object(VideoReader, "get_batch") as vr_get_batch_mocked:
        ...
        for i, (_frames, paths) in enumerate(ilids_loader):
            ...
            for ii, (call_index, call_get_batch_argument) in enumerate(
                zip(
                    range(batch_start, batch_end),
                    vr_get_batch_mocked.call_args_list[batch_start:batch_end],
                )
            ):
                ...
                assert_that(
                    sequence_frame_indices.max(initial=-1),
                    is_(less_than(sequence_frame_count)),
                )
